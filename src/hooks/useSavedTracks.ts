...
import { fetchCurrentUsersSavedTracks } from '../utils';
...
function useSavedTracks(limit = 20) {
  ...
  const [savedTracksError, setSavedTracksError] = useState<null | string>(null);
  const [savedTracksLoading, setSavedTracksLoading] = useState<boolean>(false);
