...
import { fetchCurrentUsersTopArtistsOrTracks } from '../utils';
import { FetchCurrentUsersTopArtistsOrTracksArgs } from '../utils/fetchCurrentUsersTopArtistsOrTracks';
 ...
 */
function useTopTracksOrArtists({
  type = 'tracks',
  time_range = 'short_term',
  limit = 20,
}: FetchCurrentUsersTopArtistsOrTracksArgs) {
  const [topTracks, setTopTracks] = useState([]);
  const [topTracksError, setTopTracksError] = useState<null | string>(null);
  const [topTracksLoading, setTopTracksLoading] = useState<boolean>(false);

  async function fetchData() {
    setTopTracksLoading(true);
    const tracks = await fetchCurrentUsersTopArtistsOrTracks({
      type,
      time_range,
      limit,
    });
...
export default useTopTracksOrArtists;
