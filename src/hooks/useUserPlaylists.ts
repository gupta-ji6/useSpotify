...
import { fetchCurrentUserPlaylists } from '../utils';

/**
 * Custom react hook to get a list of the playlists owned or followed by the current Spotify user.
...
function useUserPlaylists(limit = 20) {
  ...
  const [userPlaylistsError, setUserPlaylistsError] = useState<null | string>(null);
  const [userPlaylistsLoading, setUserPlaylistsLoading] = useState<boolean>(false);
